...
contract Exchange {
    ...
    function addLockedLiquidity(
        ...
        // address payable(creatorsToken).transfer(address(this), _tokenAmount);

        lockedLiquidityDeposits[msg.sender][
        ...
        lockedLiquidityDeposits[msg.sender][
            lockedLiquidityLastDepositId[msg.sender]
        ].nativeTokenAmount = _tokenAmount;
        lockedLiquidityDeposits[msg.sender][
            lockedLiquidityLastDepositId[msg.sender]
        ].lockedUntil = block.timestamp + _lockedFor;
        lockedLiquidityLastDepositId[msg.sender]++;
    }

    function unlockLockedLiquidity(uint _depositId) public {
        require(
            lockedLiquidityDeposits[msg.sender][_depositId].lockedUntil <=
                block.timestamp,
            "liquidity lock duration has not passed yet"
        );
        LockedLiquidityDeposit memory lockedLiquidity = lockedLiquidityDeposits[
            msg.sender
        ][_depositId];
    ...
    function withdrawLiquidity(
        uint _creatorsTokenAmount,
        uint _nativeTokenAmount
    ) public {
        require(
            creatorsTokenDeposited[msg.sender] <= _creatorsTokenAmount,
            "address does not have enough creators tokens deposited"
        );
        require(
            nativeTokenDeposited[msg.sender] <= _nativeTokenAmount,
            "address does not have enough native tokens deposited"
        );
    ...
    function getDepositInfo(
        address _depositer,
        uint _id
    ) public view returns (LockedLiquidityDeposit memory) {
